/* ============================================
   CHARTS.JS — Histogram, Category, Scatter, Heatmap
   ============================================ */
  ...
  activeChart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        x: {
          grid: { display: false },
          ticks: { maxRotation: 35, font: { size: 10 }, maxTicksLimit: 10 },
        },
        y: {
  ...
  activeChart = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: (item) => `(${item.parsed.x.toFixed(2)}, ${item.parsed.y.toFixed(2)})`,
          },
        },
      },
      scales: {
        x: {
          title: { display: true, text: colX, font: { size: 11 } },
          grid:  { color: 'rgba(0,0,0,0.05)' },
        },
        y: {
          title: { display: true, text: colY, font: { size: 11 } },
          grid:  { color: 'rgba(0,0,0,0.05)' },
        },
      },
    },
...
export function renderHeatmap(data, numericCols) {
  ...
  if (numericCols.length < 2) {
    ...
    if (wrap) wrap.innerHTML = '<p style="color:var(--color-text-muted);text-align:center;padding:40px;font-size:13px;">Need at least 2 numeric columns for a correlation heatmap.</p>';
    return;
  }

  const matrix = correlationMatrix(data, numericCols);
  const matrixData = [];
  numericCols.forEach((rowLabel, r) => {
    numericCols.forEach((colLabel, c) => {
      matrixData.push({ x: colLabel, y: rowLabel, v: matrix[r][c] });
    });
  });
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            ...
            label: (item) => {
              const v = item.dataset.data[item.dataIndex]?.v ?? 0;
...
/* ---- Heatmap colour scale: red(−1) → white(0) → amber(+1) ---- */
function heatColor(v) {
  ...
  if (clamped >= 0) {
    ...
    const t = clamped;
    const r = 255;
    const g = Math.round(255 - t * (255 - 158));
