...
export function exportMarkdown(summary, insights, mlResult, filename) {
  ...
  const mlMD = mlResult
    ? `## ML Recommendations\n\n**Task:** ${mlResult.taskDesc}\n**Target Column:** ${mlResult.targetColumn}\n**Readiness Score:** ${mlResult.readiness}/100\n\n### Top Models\n${mlResult.models.map(m => `- **${m.name}** (score: ${m.score}) — ${m.note}`).join('\n')}`
    : '';
...
export async function exportPDF(filename, summary, stats, insights, mlResult) {
  ...
  try {
    ...
    const bodyText = (text, size = 9, color = [55, 65, 81]) => {
      doc.setFont('helvetica', 'normal');
    ...
    stats.forEach((s, idx) => {
      ...
      xOff = margin;
      vals.forEach((v, i) => {
        const str = String(v ?? '—').substring(0, 14);
        doc.text(str, xOff, y);
        xOff += colWidths[i];
      });
      y += 6;
    ...
    if (mlResult) {
      ...
      bodyText(`Target Column: ${mlResult.targetColumn}  |  Readiness: ${mlResult.readiness}/100`);
      y += 2;
      mlResult.models.forEach(m => {
        bodyText(`• ${m.name} (score: ${m.score}) — ${m.note}`, 8.5);
      });
    }
