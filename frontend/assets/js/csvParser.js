    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        if (results.errors.length && results.data.length === 0) {
          reject(new Error('Failed to parse CSV: ' + results.errors[0].message)); return;
        }
        resolve({ data: results.data, columns: results.meta.fields || [], filename: file.name });
      },
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        resolve({ data: raw, columns: raw.length > 0 ? Object.keys(raw[0]) : [], filename: file.name });
    ...
    reader.onload = (e) => {
      try {
        ...
        if (!Array.isArray(parsed)) {
          ...
          else throw new Error('JSON must be an array or object with an array field.');
        }
        resolve({ data: parsed, columns: parsed.length > 0 ? Object.keys(parsed[0]) : [], filename: file.name });
...
function buildIrisDataset() {
  ...
  const data = Array.from({ length: 150 }, (_, i) => {
    const cls = Math.floor(i/50), m = means[cls];
    return {
